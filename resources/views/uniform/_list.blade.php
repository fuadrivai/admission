    ...
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body">
            <div class="row align-items-center mb-2 gy-2">
                <div class="col-auto">
                    <div class="student-avatar bg-primary text-white rounded-circle d-inline-flex align-items-center justify-content-center fw-bold fs-5" style="width: 45px; height: 45px;">
                ...
                <div class="col-md-auto text-md-end">
                    <button type="button" 
                            class="btn btn-sm btn-secondary view-order-details me-1" 
                            data-id="{{ $order->id }}" 
                            data-details="{{ json_encode($order->details) }}"
                            title="Quick View Order Items">
                        <i class="fa fa-folder me-1"></i> Items ({{ $order->total_items }})
                    </button>
                    <a href="/uniform/{{ $order->id }}" class="btn btn-sm btn-primary me-1" title="View Order Detail Page">
                        <i class="fa fa-eye me-1"></i> View Detail
                    </a>
                    @if($order->order_link && !in_array($status, ['PAID', 'SETTLED', 'COMPLETED']))
                        <a href="{{ $order->order_link }}" target="_blank" class="btn btn-sm btn-success" title="Payment Link">
                    ...
                    <div class="academic-info">
                        <div class="info-group">
                            ...
                            <div class="info-value small text-dark">
                                ...
                                Level / Grade: {{ $order->level_name ?? $order->level->name ?? '-' }} / {{ $order->grade_name ?? $order->grade->name ?? '-' }}
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="academic-info">
                        <div class="info-group">
                            ...
                            <div class="info-value small text-dark">
                                ...
                                Total Amount: <span class="fw-bold text-success">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span><br>
                                @if($order->payment_date)
                                    Payment Date: {{ \Carbon\Carbon::parse($order->payment_date)->format('d M Y H:i') }}
                                @else
